...
/**
 * Records each step: save PNG under the run’s screenshot folder, optionally upload to ImgBB,
 * append a CSV row, and regenerate {@code TestReport.html} so the dashboard stays current.
 */
public final class TestReporter {
    ...
    /** Same as {@link #takeScreenshot(WebDriver, AutomationRun, String, boolean, String)} with pass + generic detail text. */
    public static void takeScreenshot(WebDriver driver, AutomationRun run, String title) throws IOException {
        takeScreenshot(driver, run, title, true, "Step completed successfully");
    }

    /**
     * Increments {@link AutomationRun} step counters, writes the image file, uploads (best effort),
     * then updates CSV + HTML report.
     */
    public static void takeScreenshot(WebDriver driver, AutomationRun run, String title, boolean isPass, String details)
            throws IOException {
        run.totalSteps++;
        if (isPass) {
            run.passedSteps++;
        ...
        File folder = new File(run.ssDir);
        if (!folder.exists()) {
            folder.mkdirs();
        }
        ...
        writeCsv(run, timestamp, title, uploadedUrl, outputFile.getName());
        writeHtmlReport(run, timestamp, title, outputFile.getName(), uploadedUrl, isPass, details);
    }

    /**
     * Chrome-only full-length capture via CDP; other drivers fall back to viewport screenshot.
     * Useful for long checkout pages; not wired from {@link org.example.Main} by default.
     */
    public static void takeFullPageScreenshot(WebDriver driver, AutomationRun run, String title) throws IOException {
        run.totalSteps++;
        run.passedSteps++;
        ...
        writeCsv(run, timestamp, title, uploadedUrl, outputFile.getName());
        writeHtmlReport(run, timestamp, title, outputFile.getName(), uploadedUrl, true, "Full page capture");
    }

    /** POST multipart-style body to ImgBB; returns direct image URL from JSON (simple parse). */
    private static String uploadToImgbb(File imageFile) throws IOException {
        byte[] fileContent = Files.readAllBytes(imageFile.toPath());
        String encodedImage = Base64.getEncoder().encodeToString(fileContent);

        String data = "key=" + MahmattersConfig.imgbbApiKey
                + "&image=" + URLEncoder.encode(encodedImage, StandardCharsets.UTF_8);

        URL url = URI.create("https://api.imgbb.com/1/upload").toURL();
    ...
    /** Appends one row; creates header row the first time the CSV file is written. */
    private static void writeCsv(AutomationRun run, String timestamp, String title, String url, String localFileName) {
        File fileObj = new File(run.csvPath);
        if (!fileObj.getParentFile().exists()) {
            fileObj.getParentFile().mkdirs();
        }
        boolean fileExists = fileObj.exists();

        try (FileWriter fw = new FileWriter(run.csvPath, true);
    ...
    /**
     * Pushes this step’s HTML block onto {@link AutomationRun#htmlSteps}, then rewrites the whole
     * report file (summary + all steps) so progress is visible after every screenshot.
     */
    private static void writeHtmlReport(AutomationRun run, String timestamp, String title, String localFileName,
            String url, boolean isPass, String details) {
        File htmlFolder = new File(run.htmlDir);
        if (!htmlFolder.exists()) {
            htmlFolder.mkdirs();
        }

        String htmlFile = run.htmlDir + "\\TestReport.html";
        String relativeImgPath = "../../../screenshots/" + run.runDate + "/" + run.runTime + "/" + localFileName;
        ...
        stepHtml.append("                        <img class=\"screenshot\" src=\"").append(relativeImgPath)
                .append("\" alt=\"").append(title).append("\">\n");
        stepHtml.append("                    </a>\n");
        stepHtml.append("                </div>\n");
        ...
        run.htmlSteps.add(stepHtml.toString());

        try (FileWriter fw = new FileWriter(htmlFile, false);
                BufferedWriter bw = new BufferedWriter(fw);
                PrintWriter out = new PrintWriter(bw)) {

            double passRate = run.totalSteps > 0 ? ((double) run.passedSteps / run.totalSteps) * 100 : 0;
            String overallStatus = run.failedSteps > 0 ? "FAILED" : "PASSED";
            String statusBadgeClass = run.failedSteps > 0 ? "status-fail" : "status-pass";
            ...
            out.println("    <title>Mahj MATters Automation - Test Report</title>");
            out.println("    <style>");
            ...
            out.println("            <h1>Mahj MATters Automation</h1>");
            out.println("            <p style=\"font-size: 1.2em; margin: 10px 0;\">Test Report with Detailed Steps</p>");
            out.println("            <div class=\"timestamp\">Generated on: " + run.runDate + " at "
                    + run.runTime.replace("-", ":") + "</div>");
            out.println("        </div>");
            ...
            out.println("                <div class=\"number\">" + run.totalSteps + "</div>");
            out.println("            </div>");
            out.println("            <div class=\"summary-card\">");
            out.println("                <h3>Passed</h3>");
            out.println("                <div class=\"number\" style=\"color: #28a745;\">" + run.passedSteps + "</div>");
            out.println("            </div>");
            out.println("            <div class=\"summary-card\">");
            out.println("                <h3>Failed</h3>");
            out.println("                <div class=\"number\" style=\"color: #dc3545;\">" + run.failedSteps + "</div>");
            out.println("            </div>");
            out.println("            <div class=\"summary-card\">");
            ...
            out.println("            <p><strong>Test Duration:</strong> " + run.startTime + " to " + currentTime + "</p>");
            out.println("        </div>");
