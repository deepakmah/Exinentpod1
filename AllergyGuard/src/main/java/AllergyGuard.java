...
/**
 * National Allergy multi-store Magento automation (Chrome, eager page load for heavy storefronts, sitemap, cart/checkout, Klaviyo, reports).
 * Default substore: {@value #DEFAULT_SITE_BASE} ([Allergy Guard Direct](https://www.allergyguarddirect.com/)).
 * Use {@code -DallergyGuard.site.base=https://www.allergycontrol.com} for [Allergy Control Products](https://www.allergycontrol.com/).
 * <p>
 * JVM: {@code -DallergyGuard.site.base}, {@code -DallergyGuard.sitemap.url}, optional {@code -Dwebdriver.chrome.driver}.
 * Legacy {@code allergyControl.site.base} / {@code allergyControl.sitemap.url} are still read if the new names are unset.
 * Args: optional leading {@code http(s)://} store URL; optional numeric seed for shuffling product order.
 */
public class AllergyGuard {
    ...
    static String OUTPUT_ROOT = "C:\\Users\\deepa\\Documents\\Automation\\AllergyGuard";
    static String SS_DIR = OUTPUT_ROOT + "\\screenshots\\" + RUN_DATE + "\\" + RUN_TIME;
    static String HTML_DIR = OUTPUT_ROOT + "\\html\\" + RUN_DATE + "\\" + RUN_TIME;
    static String CSV_PATH = OUTPUT_ROOT + "\\AllergyGuard.csv";
    ...
    static final String LOG_PREFIX = "[AllergyGuard]";
    static final String PROP_SITE_BASE = "allergyGuard.site.base";
    static final String PROP_SITEMAP_URL = "allergyGuard.sitemap.url";
    /** Default substore URL (override with {@code -DallergyGuard.site.base} or first program arg). */
    static final String DEFAULT_SITE_BASE = "https://www.allergyguarddirect.com";
    ...
    private static String[] configureSiteFromProgramArgs(String[] args) {
        String propBase = System.getProperty(PROP_SITE_BASE);
        if (propBase == null || propBase.isBlank()) {
            propBase = System.getProperty("allergyControl.site.base"); // legacy name
        }
        String base = normalizeSiteBase(
                propBase != null && !propBase.isBlank() ? propBase : DEFAULT_SITE_BASE);
        int skip = 0;
        ...
        if (sitemapProp == null || sitemapProp.isBlank()) {
            sitemapProp = System.getProperty("allergyControl.sitemap.url"); // legacy name
        }
        if (sitemapProp != null && !sitemapProp.isBlank()) {
    ...
    /** Launches Chrome with {@link PageLoadStrategy#EAGER}; {@code -Dwebdriver.chrome.driver} or WebDriverManager. */
    private static WebDriver createChromeDriver() {
        ...
        if (manualDriver != null && !manualDriver.isBlank()) {
            ...
            System.out.println(LOG_PREFIX + " ChromeDriver resolved (if this was slow, first run may have downloaded chromedriver).");
        }
        System.out.flush();
    ...
    /**
     * Waits for DOM ready, brief jQuery idle (non-blocking if trackers keep AJAX open),
     * Magento-style loading masks to hide, then Klaviyo dismiss.
     */
    private static void waitForPageFullyLoaded(WebDriver driver) {
        WebDriverWait docWait = new WebDriverWait(driver, Duration.ofSeconds(45));
        try {
            docWait.until(d -> {
            ...
            System.out.println(LOG_PREFIX + " document.readyState still not interactive/complete — continuing");
        }
        try {
            WebDriverWait jq = new WebDriverWait(driver, Duration.ofSeconds(12));
            jq.until(d -> Boolean.TRUE.equals(((JavascriptExecutor) d).executeScript(
                    "return typeof jQuery === 'undefined' || jQuery.active === 0")));
        } catch (TimeoutException e) {
            System.out.println(LOG_PREFIX + " jQuery still has active requests (common with analytics) — continuing after short pause");
            try {
                Thread.sleep(1500);
        ...
        waitForLoadingOverlaysHidden(driver, Duration.ofSeconds(20));
        dismissKlaviyoSignupIfPresent(driver);
    }

    /** Magento / Luma full-page loaders often block interaction while visible. */
    private static void waitForLoadingOverlaysHidden(WebDriver driver, Duration maxWait) {
        WebDriverWait w = new WebDriverWait(driver, maxWait);
        try {
            w.until(d -> Boolean.TRUE.equals(((JavascriptExecutor) d).executeScript(
                    "var sels = ['.loading-mask','.loader','.page-loader','#maincontent .loading-mask'];"
    ...
    private static String fetchHttpText(String urlString) throws IOException {
        ...
        conn.setRequestProperty("User-Agent", "Mozilla/5.0 (compatible; AllergyGuardAutomation/1.0)");
    ...
    /**
     * Picks Magento configurable options: visual swatches/switches first, then {@code select} dropdowns.
     * Re-queries the DOM each round so AJAX price/stock updates apply before add-to-cart.
     */
    private static void selectConfigurableOptionsIfPresent(WebDriver driver, WebDriverWait wait) throws InterruptedException {
        for (int round = 0; round < 8; round++) {
            int acted = clickSwatchOptionsForUnselectedAttributes(driver);
            acted += selectDropdownConfigurableOptions(driver, wait);
            if (acted == 0) {
                break;
            }
        }
    ...
    /**
     * For each visible {@code .swatch-attribute} block without a selected swatch, clicks the first
     * enabled option ({@code .swatch-option} not disabled/unavailable). Used on stores like Allergy Guard Direct
     * where configurables use visual switches instead of (or in addition to) dropdowns.
     */
    private static int clickSwatchOptionsForUnselectedAttributes(WebDriver driver) throws InterruptedException {
        int clicked = 0;
        List<WebElement> attributes = driver.findElements(By.cssSelector(
                "#product-options-wrapper .swatch-attribute, .product-options-wrapper .swatch-attribute"));
        for (WebElement attr : attributes) {
            try {
                if (!attr.isDisplayed()) {
                    continue;
                }
                if (!attr.findElements(By.cssSelector(".swatch-option.selected")).isEmpty()) {
                    continue;
                }
                List<WebElement> choices = attr.findElements(By.cssSelector(".swatch-option"));
                for (WebElement choice : choices) {
                    if (!choice.isDisplayed()) {
                        continue;
                    }
                    String cls = choice.getAttribute("class");
                    if (cls != null && (cls.contains("disabled") || cls.contains("unavailable"))) {
                        continue;
                    }
                    safeClick(driver, choice);
                    clicked++;
                    waitForPageFullyLoaded(driver);
                    Thread.sleep(450);
                    break;
                ...
                // next row on re-fetch
            }
        }
        return clicked;
    }

    /** For each visible configurable {@code select}, chooses the first real option value (not empty / not "0"). */
    private static int selectDropdownConfigurableOptions(WebDriver driver, WebDriverWait wait) throws InterruptedException {
        int acted = 0;
    ...
    private static void writeHtmlReport(String timestamp, String title, String localFileName, String url, boolean isPass, String details) {
        ...
        try (FileWriter fw = new FileWriter(htmlFile, false);
            ...
            out.println("    <title>AllergyGuard Automation - Test Report</title>");
            out.println("    <style>");
            ...
            out.println("            <h1>AllergyGuard Automation</h1>");
            ...
            out.println("            <p>Generated by AllergyGuard Automation Framework</p>");
            out.println("        </div>");
            out.println("    </div>");
            out.println("</body>");
            out.println("</html>");
